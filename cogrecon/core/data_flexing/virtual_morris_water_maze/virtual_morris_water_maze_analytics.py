...
import virtual_morris_water_maze_parser as log_reader


def generate_virtual_morris_water_maze_summary_file(search_directory=None,
                                                    file_regex='..._\d_\d_\d\d\d\d-\d\d-\d\d_\d\d-\d\d-\d\d.dat',
                                                    save_xyz_file=True, next_to_distance=9, next_to_point=(5, -7)):
    ...
    # Get file listing
    files = log_reader.find_data_files_in_directory(directory, file_regex=file_regex)
    ...
    with open(summary_file, 'wb') as sfp:
        ...
        for f in files:
            ...
            try:
                ...
                logging.error('There was an error reading the filename meta-information. Please confirm {0} is a '
                              'valid log file.'.format(f))
                exit()
            ...
            for idx, i in enumerate(iterations):
                ...
                if len(t) >= 2:
                    ...
                    if distance.euclidean(next_to_point, (x[-1], y[-1])) <= next_to_distance:
            ...
            line = ','.join([str(_m) for _m in [meta["subid"], meta["trial"], meta["iteration"], meta["datetime"],
                                                f, ' '.join([str(_p) for _p in next_to_point]), next_to_distance,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='When called without arguments, a directory dialog will appear to select '
    ...
    generate_virtual_morris_water_maze_summary_file(args.search_path, file_regex=args.search_regex,
                                                    save_xyz_file=args.save_xyz_file,
                                                    next_to_distance=args.next_to_distance,
                                                    next_to_point=args.next_to_point)
